;; functions written by adam doplet

;; setup message to prompt before killing emacs
...
;; find the topmost directory contain the file NAME
...
;; Cycle through buffers using Ctrl-tab ( does this work?? )
(setq switch-buf-list nil)
(setq switch-buf-timer nil)

;; something todo with buffers?
...
;; another uknown buffer function"
...
;; find the tag at the point ( is this function redundant for emacs 22? )
...
;; list of all tags searched for this session
(defvar tags-stack nil)

;; push a tags lookup onto the tags-stack
(defun tags-stack-push (el)
  "Push a tag lookup onto the tags-stack"
...
;; similar advice for tags-search
...
(defun to-column (c &optional text)
  "Insert N spaces followed by optional text"
  (interactive "nColumn: ")
  (if (null text)
      (setq text " "))
  (if (< (current-column) c)
...
;; function to create a 2 digit number string ( whats this for? )
